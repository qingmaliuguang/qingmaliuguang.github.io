...
> Spring AOP支持的自动配置。相当于在你的配置中启用@EnableAspectJAutoProxy。 如果spring.aop.auto=false，配置将不会被激活。默认情况下，proxyTargetClass属性为true，但是可以通过指定spring.aop.proxy-target-class=false来重写。
